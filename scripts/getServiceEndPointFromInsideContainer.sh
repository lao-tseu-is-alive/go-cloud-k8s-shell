...
curl -s -k -H "Authorization: Bearer $TOKEN" -H 'Accept: application/json' $APISERVER/api/v1/namespaces/$TARGET_NAMESPACE/services |jq --arg TARGET "${TARGET_SERVICE}" '.items[] | .metadata.name'
